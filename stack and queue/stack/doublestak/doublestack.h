...
{
    private:
        T *vector;  //共享栈存储空间的首地址
        int bottom[2];  //分别代表两个栈的栈底指示器。
        int top[2];         //两个栈的栈顶指示器 ,用数组有用数组的道理
        int maxsize;
    public:
        doublestack(int size=100); //构造函数，初始化栈
        bool isempty(int i) const;  //这里的i表示第i个栈
        bool isfull() const;
        bool push(int i,T elem);  //第i个栈进栈。
        bool pop(int i,T &elem);
        bool gettop(int i,T &elem) const;
        void traverse() const;
