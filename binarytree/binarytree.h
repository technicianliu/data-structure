/*只要知道了根结点的左右链，就可以方便的从根结点出发找到他的左右子树下的任一个结点。
因此，二叉树类中包括根结点，操作包括建树，插入/删除结点，遍历二叉树以及查找孩子/双亲等操作*/
#include <iostream>
#include <algorithm>
#include "treenode.h"
#include "stack.h"
...
/*二叉链表的结构中没有指向双新的链，因此，查找当前结点的双亲必须从指定结点subtree开始，
对其左右子树进行搜索，在搜索中判断搜索到的结点的左右孩子是否与单前给定结点pcurrent相同，
若相同，搜索到达的结点，即为当前结点的双亲结点；如果不同，则继续搜索。如果搜索指针为空，
则说明当前结点为根结点，没有双亲结点。*/
template<class T> treenode<T>* binarytree<T>::parent(treenode<T> *subtree,treenode<T> *pcurrent)
{
       if(!subtree)
             return NULL;
     /*这个一方面是根结点，另一方面是为了下面的左子树遍历，没有这个条件下面的递归就无法进行，
     自己写不出来就是没想到先考虑左子树，然后再去考虑右子树。*/
       if(subtree->leftchild==pcurrent||subtree->rightchild==pcurrent)
             return subtree;

       treenode<T> *temp;

       if((temp=parent(subtree->leftchild,pcurrent))!=NULL)  
     //这个思路是先把左子树遍历完，然后再考虑右子树，必须有一个变量temp，不可以直接返回，但右子树可以直接返回  
            return temp;
       else
             return parent(subtree->rightchild,pcurrent);
}

/*二叉树遍历是指遵循某种次序，依次对二叉树中每个节点均做一次且仅做一次访问。
从二叉树的定义可知，一颗非空的二叉树由根结点及左右子树这三个部分构成。
因此，在任意给定的结点上，可以按某种次序执行三个操作：
(1),访问结点本身(V).(2),遍历该结点的左子树(L).(3)，遍历该结点的右子树(R).
二叉树是一种有序树，因此，仅有VLR(前序遍历)，LVR(中序遍历)，LRV(后序遍历)，另外三种遍历是前三种的镜像。*/
